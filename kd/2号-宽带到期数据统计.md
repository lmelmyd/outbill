...
> 主机在133.160.91.22-drecv1,目录：remind_adsl

顺序执行
```sh
往月数据备份执行：remind_adsl/run_wk_adsl_exec.sh
往月数据汇总执行：remind_adsl/run_remind_sum.sh
                    p_brt_adsl_remind_proc  
```
...
BEGIN
    ...
    FOR rec IN (SELECT id_tag, source_id, eparchy_code, COUNT(1) cnt,
                 ...
                 GROUP BY id_tag, source_id, eparchy_code) LOOP
